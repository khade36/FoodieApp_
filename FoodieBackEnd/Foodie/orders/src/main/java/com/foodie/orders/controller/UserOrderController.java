...
public class UserOrderController {
    ...
    public ResponseEntity<?> deleteUserItemFromList(@PathVariable int orderId,HttpServletRequest request)
        ...
        } catch (UserNotFoundException | OrderNotFoundException | MessagingException m) {
            throw new UserNotFoundException();
        }
        return responseEntity;
    }

    @GetMapping("/user/OwnerOrders")
    public ResponseEntity<?> getItemByRestaurantName(HttpServletRequest request) throws RestaurantNotFoundException, ItemNotFoundException {
        Claims claims = (Claims) request.getAttribute("claims");
        System.out.println("emailId from claims :: " + claims.getSubject());
        String ownerId = claims.getSubject();
        try {
            responseEntity = new ResponseEntity<>(userOrderService.fetchOrderByOwnerId(ownerId), HttpStatus.OK);
        }
        catch ( UserNotFoundException e){
    ...
    @PutMapping("/changeOrderStatus/{emailId}/{orderId}")
    public ResponseEntity<?>changeOrderStatus(@PathVariable String emailId,@PathVariable int orderId)
    {
        try {
            responseEntity = new ResponseEntity<>(userOrderService.changeOrderStatus(emailId, orderId), HttpStatus.OK);
        }catch (UserNotFoundException | OrderNotFoundException | MessagingException e)
        {
            e.printStackTrace();
        }
        return responseEntity;
