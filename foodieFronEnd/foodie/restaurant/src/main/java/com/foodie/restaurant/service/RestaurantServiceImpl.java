...
public class RestaurantServiceImpl implements RestaurantService {
    ...
    @Override//working
    public RestoOwner saveRestoOwnerToList(RestoOwner restoOwner) throws RestaurantOwnerAlreadyExistException //throws RestaurantOwnerAlreadyExistException
    {
        ...
        if(restaurantRepositary.findById(restoOwner.getOwnerId()).isPresent())
        {
...
//                            restoOwner.setRestaurant(restaurants);
    ...
    @Override//working with exception
    public Restaurant updateItem(String ownerId, String restaurantName, String itemName, int newPrice) throws RestoOwnerNotFoundException, RestaurantNotFoundException, ItemNotFoundException {
        RestoOwner restoOwner=restaurantRepositary.findById(ownerId).get();
        List<Restaurant>restaurants=new ArrayList<>();
                        ...
                        else
                        {
                            ...
                            for(Item item:itemList)//Iterating through itemList to get required item by comparing itemName one by one
                            {
                               if(item.getItemName().equals(itemName))//matching itemName one by one
                                {
                                  flag1=false;
                                  for(int i=0;i<itemList.size();i++)//this is only to remove old item
                                  {
                                      if(itemList1.get(i).getItemName().equals(itemName))
                                      {
                                          itemList1.remove(i);
                                  ...
                                  for(int i =0;i<restaurants.size();i++)//this is to remove restaurant of old item
                                  {
                                      if(restaurants1.get(i).equals(restaurantName))
                                      {
                                          restaurants1.remove(i);
                                      }
                                  }
    ...
    private final String FOLDER_PATH="E:/Wave29/CapstoneProject/Backend/Resources";
